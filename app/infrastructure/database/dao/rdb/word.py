...
class WordDAO(BaseDAO[Word]):
    ...
    async def add_word(self, word: str) -> dto.Word:
        ...
        return dto.Word.model_validate(result.scalar())
    ...
    async def get_word(self, word: str) -> dto.Word:
        ...
        word = result.scalar()
        if word is not None:
            return dto.Word.model_validate(word)

    async def get_all_checked_words(self, is_checked: bool) -> list[dto.Word]:
        result = await self.session.execute(
            select(Word).where(Word.is_checked == is_checked)
        )
        adapter = TypeAdapter(list[dto.Word])
        return adapter.validate_python(result.scalars().all())

    async def get_all_words(self) -> list[dto.Word]:
        result = await self.session.execute(select(Word))
        adapter = TypeAdapter(list[dto.Word])
        return adapter.validate_python(result.scalars().all())
